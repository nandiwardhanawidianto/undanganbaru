...
import pigurabungaungu from "../assets/pigurabungaungu.png";
...
export default function Galeri({ data }) {
  ...
  // -------------------------
  // CEK LOVE STORY (TEXT WAJIB, GAMBAR OPSIONAL)
  // -------------------------
  ...
  // teks wajib → kalau teks kosong, hide, meski gambar ada
  const filteredSections = loveStorySections.filter(
    (s) => s.text && s.text.toString().trim() !== ""
  );

  const loveStoryExists = filteredSections.length > 0;

  // cek galeri
  const galeriExists =
    (Array.isArray(slidesAtas) && slidesAtas.length > 0) ||
    (Array.isArray(slidesBawah) && slidesBawah.length > 0);

  // kalau dua²nya tidak ada → hide total
  if (!galeriExists && !loveStoryExists) return null;
  ...
  const slideVariants = {
    enter: (direction) => ({ x: direction === "right" ? 100 : -100, opacity: 0 }),
    center: { x: 0, opacity: 1 },
    exit: (direction) => ({ x: direction === "right" ? -100 : 100, opacity: 0 }),
  };
  ...
  return (
    <section
      id="galeri"
      className="relative min-h-screen flex items-center justify-center p-6 backdrop-blur-sm"
    >
      <div className="max-w-4xl w-full text-center">
        {/* Title Galeri */}
        {galeriExists && (
          <motion.h2
            className="text-4xl font-bold text-ungu-500 mb-8"
        ...
        {loveStoryExists && (
          <motion.div
            className="relative bg-ungu-500 border-2 border-white rounded-xl p-6 flex flex-col gap-8 overflow-hidden shadow-[0_0_25px_rgba(123,31,162,0.45)] mt-12"
            initial={{ opacity: 0, y: 30 }}
            whileInView={{ opacity: 1, y: 0 }}
            viewport={{ once: true }}
            transition={{ duration: 0.8 }}
          >
            ...
            <img
              src={pigurabungaungu}
              alt="Bunga"
              className="absolute top-0 left-1/2 -translate-x-1/2 w-64 opacity-90 pointer-events-none z-50"
            />
